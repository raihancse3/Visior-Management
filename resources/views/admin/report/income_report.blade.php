...
<div class="row">
  <div class="col-md-12">
    <div class="panel panel-primary" data-collapsed="0">
      <form class="form-horizontal" action="{{ url('report/income-report') }}" method="GET">
...
<div class="row">
  <div class="col-md-12">
    <div class="panel panel-primary" data-collapsed="0">
        <div class="panel-heading">
          <div class="panel-title">{{ trans('message.menu.income_report') }}</div>
            ...
            <table id="purchaseList" class="table table-bordered">
              <thead>
                <tr class="bg-success">
                  ...
                  <th width="10%" class="text-center">Amount({{Session::get('currency_symbol')}})</th>
                </tr>
              </thead>
              <tbody>
                <?php
                $total = 0;
                ?>
                @foreach($incomeReportData as $data)
                <?php
                $total +=$data->amount;
                ?>
                <tr>
                  ...
                  <td class="text-center">{{ INVOICE.$data->reference }}</td>
  ...
  var from = $('#from').val();
  var to = $('#to').val();
  window.location = SITE_URL+"/report/income-report-pdf?from="+from+"&to="+to;
  });
  $('#csv').on('click', function(event){
  event.preventDefault();
  var from = $('#from').val();
  var to = $('#to').val();
  window.location = SITE_URL+"/report/income-report-csv?from="+from+"&to="+to;
  });
  </script>
  @endsection